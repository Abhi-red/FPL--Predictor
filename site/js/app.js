...
function renderExplanation() {
  ...
  const picks = (e.standout_picks || [])
    ...
    .join("");
  const elite = e.elite_note
    ? `<p class="dv-sub"><strong>Elite weight ${e.elite_weight ?? 0} (${e.elite_weight_status || "n/a"}).</strong> ${e.elite_note}</p>`
    : "";
  el.innerHTML = `
    ...
    <ul>${picks}</ul>
    <p class="dv-sub">${sourceLabel(e.source)}</p>`;
}

function sourceLabel(source) {
  if (source && source.startsWith("claude")) return `Generated by Claude (${source}).`;
  return "Generated automatically from the model's output.";
}
